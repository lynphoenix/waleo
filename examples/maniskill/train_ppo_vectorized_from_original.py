...
if __name__ == "__main__":
    ...
    if args.robot_uids == "rj2506":
        try:
            # 使用最小化配置，避免rest pose修改导致SPS下降
            import sys
            import os
            import importlib.util
            config_dir = os.path.dirname(os.path.abspath(__file__))
            config_path = os.path.join(config_dir, "rj2506_config_minimal.py")
            spec = importlib.util.spec_from_file_location("rj2506_config_minimal", config_path)
            rj2506_config = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(rj2506_config)
            rj2506_config.apply_rj2506_config_minimal()
